...
<thead>
	<tr>
		<th colspan="4" class="th" style="font-size: 12pt; background-color: #dfefff; height: 30px; text-align:center;">- ${cname} -</th>
	</tr>
	<tr>
...
<tbody>
<c:if test="${prodByCategoryList == null || empty prodByCategoryList}">
	<tr>
		<td colspan="4" class="td" align="center">현재 상품 준비중....</td>
	</tr>
</c:if>	

<c:if test="${prodByCategoryList != null && not empty prodByCategoryList}">
	<tr>
		<c:forEach var="prodvo" items="${prodByCategoryList}" varStatus="status" >
			<%-- varStatus는 반복문의 상태정보를 알려주는 attribute 이다. 
				 status.index는 0부터 시작, status.count는 1부터 시작한다. --%>
			<td class="td" align="center">
				<a href="<%= request.getContextPath() %>/prodView.do?pnum=${prodvo.pnum}">
				<img width="120px;" height="130px;" src="images/${prodvo.pimage1}"></a>
				<br/><br/><span style="background-color: navy; color: white; ">${prodvo.pspec}</span>
				<br/>${prodvo.pname}
